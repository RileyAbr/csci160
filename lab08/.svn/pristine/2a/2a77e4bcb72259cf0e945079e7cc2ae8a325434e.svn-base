...
/*  For this exercise, you will need to declare a number of multidimensional arrays based on the following instructions.
    ...
    *NOTE All of these should be declared as instance variables of the Question1 class. 
    ...
    3) Declare a multi-dimensional array that can be used to store several friends' top 3 movies (the array will store the 
    movie titles) from different genres for each decade over the last 30 years.
    4) Declare a multi-dimensional array that can be used to store the results of a multiple choice survey 
        (answers a, b, c, d, e) for a group of students. 
    5) Declare a multi-dimensional array of type Person that can be used to store a chore list for a group of people for 
...
{  
    ...
    //3
    private String[][][] friendsTop3Movies;
    
    //4
    private char[][] studentsMultiChoiceAnswers;
    
    //5
    private Person[][][] choreList;
    
    //6
    private Boolean[][] studentsTrueFalseAnswers;
    
    //7
    private int[][][][][][][][][] toTheNthDimension;
}
